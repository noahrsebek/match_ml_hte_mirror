...
make_single_causal_forest <- function(dataset,
  ...
  # fit W forest
  forest.W <- regression_forest(X, W, tune.parameters = tuned_parameters, min.node.size = 10)
  W.hat <- predict(forest.W)$predictions
  
  
  # fit Y forest
  forest.Y <- regression_forest(X, Y, tune.parameters = tuned_parameters, min.node.size = 10)
...
# running models ----
make_single_causal_forest(master_pool, "math_gpa_full", "Math GPA",
...
run_fullsample_models <- function(input_seed){
  
  # set seed
  set.seed(input_seed)
  
  # run cf for each outcome
  causal_forest_models <- make_single_causal_forest(master_pool, "math_gpa_full", "Math GPA",
...
all_fullsample_models <- future_map(seeds, run_fullsample_models,
...
all_fullsample_models %>% readr::write_rds('fullsample_mathgpa_1k_runs_minnode_10.Rds')

rm(list=ls())

# RUNNING EBERYTHING FOR GRADUATION WITH MINNODESIZE=10
set.seed(20201111)
...
all_splitsample_models %>% readr::write_rds('fullsample_graduation_1k_runs_minnode_10.Rds')
...
all_fullsample_models %>% readr::write_rds('fullsample_mathgpa_1k_runs_minnode_tuned.Rds')
...
# note: despite the names of the above (adapted) functions, this is NOT a spitsample method; it is fullsample (name it as such)
all_splitsample_models %>% readr::write_rds('fullsample_graduation_1k_runs_minnode_tuned.Rds')
