...
process_forests <- function(forests){
  ...
  for (outcome_label in names(forests)){
    ...
    forests[[outcome_label]]$calibration_plot <- forests[[outcome_label]] %>% make_calibration_plot(outcome_label)
  }
...
print_forests <- function(forest_list){
  for (i in 1:length(forest_list)){
    ...
    forest_list[[i]]$calibration_test %>% make_calibration_text() %>% cat()
    cat("^[", calibration_footnote,"] ")
    cat('\n')
    ...
    forest_list[[i]]$tuning_output$params %>% cat()
...
```{r, results='asis', include=T, echo=F, fig.height=4.4, fig.width=7,  warning=F}
